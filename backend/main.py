...
try:
    ...
    async def test_settings_connection(payload: dict):
        ...
        try:
            if provider == "openai": return await test_connection_service.test_openai(api_key)
            elif provider == "fal.ai": return await test_connection_service.test_fal(api_key)
            elif provider == "elevenlabs": return await test_connection_service.test_elevenlabs(api_key)
            elif provider == "google": return await test_connection_service.test_google(api_key)
            elif provider == "kie.ai": return await test_connection_service.test_kie(api_key)
            return {"status": "error", "message": f"Testing not supported for {provider}"}
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"Backend Crash during {provider} test: {str(e)}")
    ...
    # --- RESCUE MODE TRIGGERED ---
    app = FastAPI(title="Lumina RESCUE MODE")
    error_details = {
    ...
    async def rescue_handler(rest_of_path: str):
